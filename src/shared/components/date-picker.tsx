    ...
    <Popover>
      <PopoverTrigger asChild>
        ...
        <Button variant={buttonClass ? "secondary" : "outline"} className={cn("pl-3 text-left font-normal w-full", !field.value && "text-muted-foreground", buttonClass)}>
          {field.value ? format(field.value, "PPP") : <span>{t("form.selectDate")}</span>}
          <CalendarIcon className="ml-auto h-4 w-4 opacity-50" />
        </Button>
        {/* </FormControl> */}
      </PopoverTrigger>
      <PopoverContent className="w-auto p-0 z-1000 pointer-events-auto" align="start">
        <Calendar mode="single" initialFocus captionLayout="dropdown" selected={field.value} onSelect={field.onChange} disabled={disabledToToday ? (date) => date > new Date() || date < new Date("1900-01-01") : false} />
      </PopoverContent>
    </Popover>
  );
}
