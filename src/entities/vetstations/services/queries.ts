...
import { vetStationControllerFindAll } from "@/shared/api/api-new";
...
export function useGetVetStations(params: Record<string, unknown>, enabled?: boolean) {
  return useQuery<PaginatedEntity<VetStation>, Error>({
    ...
    queryFn: async () => vetStationControllerFindAll(params) as Promise<PaginatedEntity<VetStation>>,
    enabled,
  });
}
...
export function useGetVetStationsInfinite(search?: string) {
  return useInfiniteQuery({
    ...
    queryFn: (params) => vetStationControllerFindAll(params.pageParam) as Promise<PaginatedEntity<VetStation>>,
