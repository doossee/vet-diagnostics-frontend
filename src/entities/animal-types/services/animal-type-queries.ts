...
export function useGetAnimalTypesInfinite(params: Record<string, unknown>) {
  return useInfiniteQuery({
    queryKey: [AnimalTypesQueryKeys.ANIMAL_TYPES, ...paramsToQueryKeys(params)],
		queryFn: () => animalTypesControllerFindAll(params),
		initialPageParam: { ...params, page: 1 },
		getNextPageParam: (lastPage) => {
			const nextPage = lastPage.meta.currentPage + 1;
			const isLast = lastPage.meta.currentPage === lastPage.meta.lastPage;

			return !isLast && lastPage.meta.total ? { ...params,	page: nextPage } : null;
		},
  })
}